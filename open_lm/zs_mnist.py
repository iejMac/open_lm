...
def main(args):
    ...
    # tar_path = "/fsx/iejmac/code/open_lm/open_lm/eval_set/mnist/test_tokens/00000_clip_embeddings.tar"
    # urls = [tar_path]
    urls = list(braceexpand("/fsx/iejmac/code/open_lm/open_lm/eval_set/cifar/tokens_test/part_{0..7}.tar"))
    random.shuffle(urls)
    dataset = wds.WebDataset(urls=urls).shuffle(10000, initial=1000)

    # reps = np.load("/fsx/iejmac/code/open_lm/open_lm/eval_set/mnist/train_tokens/classes15.npy")
    reps = np.load("/fsx/iejmac/code/open_lm/open_lm/eval_set/cifar/tokens_train/classes15.npy")
