...
def main(args):
    ...
    with torch.no_grad(), autocast():
        for sample in dataset:
            numpy_array = sample["npy"]
            txt_data = sample["txt"]


            numpy_array = numpy_decoder(numpy_array)
            txt_data = txt_data.decode('utf-8')
            if txt_data.isdigit():
                label = int(txt_data)
            else:
                label = class_dict[txt_data]

            sample = prep(torch.from_numpy(numpy_array).long()).to(args.device)
    ...
    dist.barrier()
    # Assuming args.device is your CUDA device, e.g., "cuda:0"

    # Convert labels to a tensor of shape (N_EXAMPLES_WORKER,) and move to CUDA
    labels_tensor = torch.tensor(labels, dtype=torch.long).to(args.device)

    # For embeddings, ensure they're on CUDA
    embeddings_tensor = torch.cat(embeddings).to(args.device)

    # Create storage lists for gathered data, making sure they're on the same CUDA device
    embeddings_shape = embeddings_tensor.shape
    labels_shape = labels_tensor.shape

    embeddings_list = [torch.zeros(embeddings_shape, device=args.device) for _ in range(args.world_size)]
    labels_list = [torch.zeros(labels_shape, dtype=torch.long, device=args.device) for _ in range(args.world_size)]
    # Use torch.distributed.all_gather to collect embeddings and labels from all processes
    dist.barrier()
    dist.all_gather(embeddings_list, embeddings_tensor)
    dist.all_gather(labels_list, labels_tensor)
    dist.barrier()

    if args.rank == 0:
        embeddings_list = [e.cpu() for e in embeddings_list]
        labels_list = [l.cpu() for l in labels_list]
        ...
        print("SAVING")
        torch.save(embeddings_all, os.path.join(args.logs, args.name, f"{config_name}_embeddings.pt"))
        torch.save(labels_all, os.path.join(args.logs, args.name, f"{config_name}_labels.pt"))
        print("SAVED")

        # Fit lin probe test
        N_EXAMPLES = len(embeddings_all)

        train_features = embeddings_all[:int(0.8*N_EXAMPLES)]
        train_labels = labels_all[:int(0.8*N_EXAMPLES)]
        test_features = embeddings_all[int(0.8*N_EXAMPLES):]
        test_labels = labels_all[int(0.8*N_EXAMPLES):]

        print(train_features.shape, train_labels.shape)

        # Perform logistic regression
        classifier = LogisticRegression(random_state=0, C=10, max_iter=2000, verbose=1)
