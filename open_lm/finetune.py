...
class ClsWrapper(torch.nn.Module):
    def __init__(self, lm, n_cls=174):
        super().__init__()
        self.lm = lm
        self.cls_head = torch.nn.Linear(lm.params.dim, n_cls, bias=False)
    def forward(self, x):
        pred, emb = self.lm(x)

        cls_pred = self.cls_head(emb[:, -1])
...
def main(args):
    ...
    with autocast():
        loss_value = torch.tensor(0.0).to(args.device)
        for i, sample in enumerate(dataset):
            ...
            lm_logits, cls_logits = model(batch_x)
            lm_loss_value = loss(lm_logits.reshape(-1, args.vocab_size), batch_y.reshape(-1))
            cls_loss_value = loss(cls_logits.reshape(-1, 174), torch.tensor([label]).to(args.device))

            loss_value += cls_loss_value + lm_loss_value * 0.0

            if (i+1) % args.accum_freq == 0:
                loss_value = loss_value / args.accum_freq
                if args.rank == 0:
            ...
            if i > N_EXAMPLES_WORKER:
                break


    # Partition the WebDataset across workers
    urls = list(braceexpand.braceexpand(args.val_data))
    pipeline = [wds.SimpleShardList(urls)]
    pipeline.extend([
        wds.split_by_node,
        wds.split_by_worker,
