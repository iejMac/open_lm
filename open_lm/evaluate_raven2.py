...
def main(args):
    ...
    directory_path = "/fsx/iejmac/code/open_lm/open_lm/eval_set/raven/alt_small/*.npy"

    # Function to preprocess each numpy array
    def prep(tensor):
        # Initialize a list to hold the tensors for the batch
        batch_tensors = []
        
        # The first 8 frames are common for all elements in the batch
        common_frames = tensor[:1]
        
        # Append each of the last 8 frames to the common frames to create the batch
        for i in range(1, 9):
            # Append the i-th frame from the last 8 to the common frames
            appended_tensor = torch.cat((common_frames, tensor[i].unsqueeze(0)), dim=0)
            
            # Append 1024 to each row and add to the batch list
            to_append = torch.full((2, 1), 1024)
            batch_tensor = torch.cat((appended_tensor, to_append), dim=1)
            batch_tensors.append(batch_tensor.reshape(-1))
        
        # Stack the list of tensors along a new dimension to create the batch
    ...
    with torch.no_grad(), autocast():
        # Loop over the numpy files in the directoryk

        # Make prompt
        prompt = []
        for file_path in glob.glob(directory_path)[:1]:
            # Load the numpy array and convert to a PyTorch tensor
            tensor = torch.from_numpy(np.load(file_path)).long()

            # Preprocess the tensor to create the batch
            batch = prep(tensor).to(args.device)
            prompt.append(batch[-1])
        prompt = torch.stack(prompt)

        for file_path in glob.glob(directory_path)[7:]:
            # Load the numpy array and convert to a PyTorch tensor
            tensor = torch.from_numpy(np.load(file_path)).long()
            ...
            # Extract input (x) and target (y) for the model
            losses = []
            for i in range(8):
                prompted_batch = torch.cat([prompt, batch[i:i+1]]).reshape(-1)
                batch_x = prompted_batch[None, :-1]
                batch_y = prompted_batch[1:].reshape(-1)

                # Iterate over the batch and evaluate the model
                out, _ = model(batch_x)
            ...
            classification_loss = loss(losses.reshape(1, -1), torch.tensor([7]))
