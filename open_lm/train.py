...
def finetune_one_epoch(
    ...
    for i, batch in enumerate(dataloader):
        ...
        texts, cls = batch
        texts = torch.LongTensor(texts).to(device)
        cls_targets = torch.LongTensor(cls).to(device)
        ...
        with autocast():
            ...
            out, cls_out = model(inputs)

            if args.log_logit_mean:
                logit_m.update(torch.mean(out).item())

            # lm_loss = loss(out.reshape(-1, args.vocab_size), targets.reshape(-1))
            lm_loss = F.cross_entropy(out.reshape(-1, args.vocab_size), targets.reshape(-1), reduction='none')
            cls_loss = loss(cls_out, cls_targets)

            not_pad_mask = ~(targets.reshape(-1) == args.vocab_size - 1)
...
def evaluate_cls(model, data, start_epoch, args, writer):
    """
    ...
    for i, batch in enumerate(dataloader):
        texts, cls_targets = batch
        texts = torch.LongTensor(texts).to(device)
        cls_targets = torch.LongTensor(cls_targets).to(device)
        ...
        with autocast():
            ...
            out, cls_out = model(inputs)
            preds = cls_out.argmax(dim=-1).long()

            acc = (preds == cls_targets).float().mean()
            ...
            cls_loss_m.update(cls_loss.item(), inputs.shape[0])
    ...
    log_data = {
        "loss": losses_m.avg,
        "lm_loss": lm_loss_m.avg,
        "cls_loss": cls_loss_m.avg,
        "acc": acc_m.avg,
    ...
    if is_master(args):
        print(f"evaluation perplexity: {math.exp(lm_loss_m.avg)}")
    return log_data
